...
EventLoop::EventLoop(): looping_(false), quit_(false), calling_pending_functors_(false), threadId_(Current_thread::tid()), poller_(Poller::new_defaultPoller(this)), wakeup_fd(CreateEventfd()), wakeup_channel_(new Channel(this, wakeup_fd))
{
...
void EventLoop::run_in_loop(Functor cb)
{
    ...
    else // 在其他线程执行cb，唤醒loop所在线程执行cb 为什么有其他线程
    {
        queue_in_loop(cb);
    }
}
